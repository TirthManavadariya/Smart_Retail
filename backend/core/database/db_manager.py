...
Relocated to backend/core/database/db_manager.py
"""
import sqlite3
import sys
...
# backend/core/database/ → .parent = core/database → .parent = core → .parent = backend
...
class DatabaseManager:
    ...
    def insert_many(self, table: str, data_list: list[dict]) -> None:
        ...
        query = f"INSERT INTO {table} ({columns}) VALUES ({placeholders})"
        self.execute_many(query, [tuple(d.values()) for d in data_list])
    ...
    def get_recent_detections(self, store_id: str, limit: int = 100) -> list:
        return self.fetch_all("detections", where="store_id = ?", params=(store_id,),
                              order_by="detected_at DESC", limit=limit)

    def get_pos_data(self, store_id: str = "", sku_id: str = "") -> list:
        where_parts, params = [], []
        if store_id: where_parts.append("store_id = ?"); params.append(store_id)
        if sku_id:   where_parts.append("sku_id = ?");   params.append(sku_id)
        return self.fetch_all("pos_transactions", " AND ".join(where_parts), tuple(params), order_by="date ASC")

    def get_forecasts(self, sku_id: str, store_id: str) -> list:
        return self.fetch_all("forecasts", where="sku_id = ? AND store_id = ?",
                              params=(sku_id, store_id), order_by="forecast_date ASC")

    def get_compliance_reports(self, store_id: str = "") -> list:
        where = "store_id = ?" if store_id else ""
        return self.fetch_all("compliance_reports", where, (store_id,) if store_id else (), order_by="checked_at DESC")

    def get_replenishment_orders(self, status: str = "") -> list:
        where = "status = ?" if status else ""
        return self.fetch_all("replenishment_orders", where, (status,) if status else (), order_by="created_at DESC")
