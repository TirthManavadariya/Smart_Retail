...
Relocated to backend/core/alerts/alert_manager.py
"""
import sys
from pathlib import Path
# backend/core/alerts/ → .parent = core/alerts → .parent = core → .parent = backend
...
class AlertManager:
    ...
    def create_alert(
        ...
        Returns Alert object, or None if suppressed as duplicate.
        """
        ...
        corrective = generate_corrective_action(
            alert_type=alert_type, sku_id=sku_id, aisle_id=aisle_id, shelf_id=shelf_id,
        )

        alert = Alert(
            alert_type=alert_type, severity=severity, store_id=store_id,
            aisle_id=aisle_id, shelf_id=shelf_id, sku_id=sku_id,
            message=message, revenue_impact=round(revenue_impact, 2),
            suggested_action=suggested, corrective_action=corrective,
            priority_score=round(priority_score, 2), created_at=now.isoformat(),
        )
    ...
    def save_alert(self, alert: Alert) -> int:
        ...
        try:
            ...
            return db.insert("alerts", {
                ...
                "store_id": alert.store_id, "aisle_id": alert.aisle_id,
                "shelf_id": alert.shelf_id, "sku_id": alert.sku_id,
                "message": alert.message, "revenue_impact": alert.revenue_impact,
                "suggested_action": alert.suggested_action, "priority_score": alert.priority_score,
            })
    ...
    def generate_sample_alerts(self, store_id: str = "STORE01", count: int = 10) -> list:
        ...
        for i in range(min(count, len(templates))):
            atype, msg, sku, sev, impact = templates[i]
            aid = f"A{np.random.randint(1, 7):02d}"
            sid = f"S{np.random.randint(1, 5):02d}"
            corrective = generate_corrective_action(alert_type=atype, sku_id=sku, aisle_id=aid, shelf_id=sid)
            alert = Alert(
                alert_id=i + 1, alert_type=atype, severity=sev, store_id=store_id,
                aisle_id=aid, shelf_id=sid, sku_id=sku, message=msg,
