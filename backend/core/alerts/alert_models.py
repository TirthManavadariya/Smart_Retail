...
Relocated to backend/core/alerts/alert_models.py — no sys.path needed (no local imports).
"""
...
class Alert:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "severity": self.severity, "store_id": self.store_id,
            "aisle_id": self.aisle_id, "shelf_id": self.shelf_id,
            "sku_id": self.sku_id, "message": self.message,
            "revenue_impact": self.revenue_impact, "suggested_action": self.suggested_action,
            "corrective_action": self.corrective_action, "priority_score": self.priority_score,
            "acknowledged": self.acknowledged, "created_at": self.created_at,
        }

    @staticmethod
    def from_dict(data: dict) -> "Alert":
        return Alert(
            alert_id=data.get("alert_id", 0), alert_type=data.get("alert_type", ""),
            severity=data.get("severity", 1), store_id=data.get("store_id", ""),
            aisle_id=data.get("aisle_id", ""), shelf_id=data.get("shelf_id", ""),
            sku_id=data.get("sku_id", ""), message=data.get("message", ""),
...
def generate_corrective_action(
    ...
    qty = reorder_qty if reorder_qty > 0 else 12

    if alert_type == "STOCKOUT":
        return (f"Restock {product} at {location}. "
        ...
        return f"Move {product} from {cur} to {cor} per planogram layout."
    elif alert_type == "LOW_STOCK":
        hrs = hours_to_stockout if hours_to_stockout > 0 else 4
        return (f"Schedule replenishment for {product} within {hrs} hours to avoid stockout. "
        ...
        return f"Update price tag at {location} from {det} to {exp}."
